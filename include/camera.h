...
struct camera_settings
{
  ...
  camera_settings(std::string cam_name_p, std::string cam_info_path_p,
                  ...
                  float fixed_exp_p, bool auto_gain_p, float gain_p,
                  float max_gain_p, float min_gain_p, bool enable_gamma_p,
                  float gamma_p, int binning_p, int binning_mode_p,
                  int lighting_mode_p, int auto_exposure_priority_p,
                  bool exp_comp_flag_p)
  {
    cam_name = cam_name_p;
...
class blackfly_camera
{
 public:
  blackfly_camera(camera_settings settings, CameraPtr cam_ptr)
  {
    ...
    m_cam_pub =
        m_image_transport_ptr->advertiseCamera(m_cam_settings.cam_name, 10);
    m_cam_info_mgr_ptr =
    ...
    m_image_event_handler_ptr = new ImageEventHandler(
  ...
  void set_buffer_size(unsigned int buff_size)
  {
    ...
    CEnumerationPtr ptrHandlingMode =
    ...
    CEnumerationPtr ptrStreamBufferCountMode =
        sNodeMap.GetNode("StreamBufferCountMode");
    if (!IsAvailable(ptrStreamBufferCountMode) ||
        !IsWritable(ptrStreamBufferCountMode))
    {
      ROS_ERROR(
          "Unable to set Buffer Count Mode (node retrieval). Aborting...");
    }
    CEnumEntryPtr ptrStreamBufferCountModeManual =
        ptrStreamBufferCountMode->GetEntryByName("Manual");
    if (!IsAvailable(ptrStreamBufferCountModeManual) ||
        !IsReadable(ptrStreamBufferCountModeManual))
    {
      ROS_ERROR(
          "Unable to set Buffer Count Mode entry (Entry retrieval). "
          "Aborting...");
    }
    ...
    {
      ROS_ERROR(
          "Unable to set Buffer Count (Integer node retrieval). Aborting...");
    }
    // Display Buffer Info
  ...
  void setup_camera()
  {
    try
    {
      ...
      if (m_cam_settings.is_auto_exp)
      {
        ...
        m_cam_ptr->AutoExposureExposureTimeUpperLimit =
            m_cam_settings.max_auto_exp_time;
        m_cam_ptr->AutoExposureExposureTimeLowerLimit =
            m_cam_settings.min_auto_exp_time;
      }
      else
      {
    ...
    catch (Spinnaker::Exception &ex)
    {
      ...
      std::cout << "Error code " << ex.GetError() << " raised in function "
                << ex.GetFunctionName() << " at line " << ex.GetLineNumber()
                << "." << std::endl;
    }
  }
