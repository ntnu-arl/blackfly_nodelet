...
class DeviceEventHandler : public DeviceEvent
{
 public:
  DeviceEventHandler(CameraPtr cam_ptr)
  {
    ...
    try
    {
      ...
      {
        ROS_ERROR(
            "Blackfly Nodelet: Unable to retrieve event selector entries. "
            "Aborting");
        return;
      }
      NodeList_t entries;
      ...
      for (unsigned int i = 0; i < entries.size(); i++)
      {
        ...
        if (ptrEnumEntry->GetDisplayName() == "Exposure End")
        {
          ...
          CEnumerationPtr ptrEventNotification =
              node_map.GetNode("EventNotification");
          if (!IsAvailable(ptrEventNotification) ||
              !IsWritable(ptrEventNotification))
          {
            continue;
          }
          // Retrieve entry node to enable device event
          CEnumEntryPtr ptrEventNotificationOn =
              ptrEventNotification->GetEntryByName("On");
          if (!IsAvailable(ptrEventNotification) ||
              !IsReadable(ptrEventNotification))
          {
            continue;
          }
  ...
  ~DeviceEventHandler()
  {
    // this is how the spinnaker examples release the camera pointer and allow
    // the nodelet to exit cleanly
    m_cam_ptr = nullptr;
  }
 ...
 private:
  ...
  // Camera pointer to spinnaker camera object (used to get the current exposure
  // time)
  CameraPtr m_cam_ptr;
};
