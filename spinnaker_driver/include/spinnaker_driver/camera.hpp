...
public:
  Camera(CameraSettings settings, CameraPtr cam_ptr, ros::NodeHandle & pnh)
  {
    ...
    camera_info_manager::CameraInfoManager cam_info_mgr(
      pnh, settings_.cam_name, settings_.cam_info_path);
    ...
    image_event_handler_ptr_ = std::make_unique<ImageEventHandlerImpl>(
      settings_.cam_name, cam_ptr_, cam_pub_, cam_info_msg, device_event_handler_ptr_,
      settings_.exp_comp_flag);
  ...
  void setup_camera()
  {
    try {
      ...
      CIntegerPtr ptrDeviceLinkThroughputLimit =
        cam_ptr_->GetNodeMap().GetNode("DeviceLinkThroughputLimit");
      if (!IsAvailable(ptrDeviceLinkThroughputLimit) || !IsWritable(ptrDeviceLinkThroughputLimit)) {
        std::cout << "Unable to set device link throughput limit (node retrieval; camera "
                  << settings_.cam_name << "). Aborting..." << std::endl;
      }
