...
namespace blackfly
{
  ...
  void blackfly_nodelet::onInit()
  {
    ...
    if (camera_serials.size() != num_cameras_listed ||
        ...
        exp_comp_flags.size() != num_cameras_listed || device_link_throughput_limits.size() != num_cameras_listed)
    {
    ...
    for (int i = 0; i < camera_names.size(); i++)
    {
      ...
      try
      {
        ...
        if (!cam_ptr->IsValid())
        {
          ROS_FATAL("Failed to get camera pointer from spinnaker for camera serial: %s",
      ...
      camera_settings settings(
          ...
          binning_mode[i], lighting_mode[i], auto_exposure_priority[i], exp_comp_flags[i], device_link_throughput_limits[i]);
      ...
      ROS_INFO("Successfully launched camera : %s, Serial : %s", settings.cam_name.c_str(),
               camera_serials[i].c_str());
    }
